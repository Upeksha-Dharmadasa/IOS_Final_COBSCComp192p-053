...
class OrderTableViewController: UIViewController , UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            let orderCell = OrderTblView.dequeueReusableCell(withIdentifier: "orderscell", for: indexPath) as! OrderTableViewCell
                        ...
                        if let orderInfo = item.value as? [String: Any]{
                            print(orderInfo)
                                    let status = orderInfo["Ostatus"] as? String;
                                        if( "New" == status)
                                        {
                                            self.newOrders.append(NewOrder(id: orderInfo["Oid"] as? String,cusName: orderInfo["Cname"] as? String, status: orderInfo["Ostatus"] as? String));
                                            
                                        }
                                        else if("Confirmed" == status){
                                            self.readyOrder.append(ReadyOrder(id: orderInfo["Oid"] as? String,cusName: orderInfo["Cname"] as? String, status: orderInfo["Ostatus"] as? String));
                                        }
                                    }
                                }
                                self.OrderTblView.reloadData()
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
            ...
            let vc = storyboard.instantiateViewController(identifier: "ORDER_DETAILS" )
